...
use App\Actions\Payment\PayWithSwift;
...
class Payment extends StepComponent
{
    ...
    public function save(): void
    {
        ...
        $totalPrice = $cartTotal + $shippingPrice + $prescriptionFee;

        session()->put('checkout.totalPrice', $totalPrice);

        $order = (new CreateOrder)->handle();

        match (data_get(session()->get('checkout'), 'payment')[0]['slug']) {
            PaymentType::SwiftTransfer() => (new PayWithSwift())->handle($order),
        };
    }
